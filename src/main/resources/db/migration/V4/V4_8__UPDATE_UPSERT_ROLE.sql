-- 1️⃣ Drop the existing variants
DROP FUNCTION IF EXISTS upsert_role(VARCHAR, BIGINT, TEXT, BIGINT);
DROP FUNCTION IF EXISTS upsert_role(VARCHAR, BIGINT, TEXT);
DROP FUNCTION IF EXISTS upsert_role(VARCHAR, BIGINT, VARCHAR);

-- 2️⃣ Recreate the clean version
